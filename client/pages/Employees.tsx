...
const Employees: React.FC = () => {
  ...
  const [selectedEmp, setSelectedEmp] = useState<Employee | null>(null); // For detail view/edit
  ...
  const handleExport = () => {
    const csvContent = "data:text/csv;charset=utf-8," 
      ...
      + employees.map(e => `${e.id},${e.name},${e.email},${e.role},${e.jobPosition},${e.dateHired}`).join("\n");
  ...
  const handleOpenModal = (emp?: Employee) => {
    if (emp) {
      ...
      setFormData({ role: 'EMPLOYEE', avatarUrl: `https://picsum.photos/200?random=${Date.now()}` });
      setIsEditMode(false);
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isEditMode && formData.id) {
      await db.updateUser(formData as Employee);
    } else {
      await db.addUser(formData as Employee);
    }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(emp => (
              <tr key={emp.id} className="hover:bg-mint-50/50 transition-colors">
                <td className="p-4 flex items-center gap-3 cursor-pointer" onClick={() => setSelectedEmp(emp)}>
                ...
                <td className="p-4 text-sm text-gray-500 hidden md:table-cell">{emp.dateHired}</td>
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={() => handleDelete(emp.id)} className="p-2 hover:bg-red-50 rounded-full text-red-500"><Trash2 size={16} /></button>
                  </div>
                </td>
              </tr>
            ))}
        ...
        {filtered.length === 0 && <div className="p-8 text-center text-gray-500">No employees found.</div>}
      </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-2xl">
            <h2 className="text-xl font-bold mb-4">{isEditMode ? 'Edit Employee' : 'New Employee'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <select className="input-std" value={formData.role} onChange={e => setFormData({...formData, role: e.target.value as any})}>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" className="px-4 py-2 bg-mint-600 text-white rounded-lg hover:bg-mint-700">Save</button>
              </div>
            </form>
          </div>
        </div>
          ...
          <div className="bg-white w-full max-w-md h-full shadow-2xl overflow-y-auto p-6 animate-slide-in">
             <div className="flex justify-between items-start mb-6">
               <div className="flex items-center gap-4">
                  <img src={selectedEmp.avatarUrl} className="w-16 h-16 rounded-full border-2 border-mint-200" />
                  <div>
                    <h2 className="text-xl font-bold">{selectedEmp.name}</h2>
                    <p className="text-gray-500">{selectedEmp.jobPosition}</p>
                  </div>
               </div>
               <button onClick={() => setSelectedEmp(null)}><User size={24} className="text-gray-400" /></button>
             </div>
             ...
             <div className="flex border-b border-gray-200 mb-6">
                {['Personal', 'Job', 'Time Off', 'Training'].map(tab => (
                  <button 
                    ...
                    className={`flex-1 pb-3 text-sm font-medium ${activeTab === tab ? 'text-mint-600 border-b-2 border-mint-600' : 'text-gray-500'}`}
                  >
                    {tab}
                  </button>
                ))}
