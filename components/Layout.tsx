...
const Layout = () => {
  ...
  const navItems = [
    ...
    { name: 'Employees', path: '/employees', icon: <Users size={20} />, allowed: ['ADMIN'] }, // Admin only usually
    { name: 'Time Off', path: '/time-off', icon: <Calendar size={20} />, allowed: ['ADMIN', 'EMPLOYEE'] },
  ...
  return (
    <div className="flex h-screen bg-mint-50">
      {/* Sidebar - Desktop */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white shadow-lg transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex flex-col h-[calc(100%-4rem)] justify-between">
          <nav className="flex-1 px-4 py-6 space-y-2">
            {navItems.filter(item => item.allowed.includes(user?.role || '')).map((item) => (
              <NavLink
                key={item.path}
                to={item.path}
          ...
          <div className="p-4 border-t border-gray-100">
            <div className="flex items-center gap-3 mb-4 px-4">
              <img src={user?.avatarUrl} alt="Avatar" className="w-10 h-10 rounded-full object-cover ring-2 ring-mint-200" />
