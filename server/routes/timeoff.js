...
router.get("/", async (req, res) => {
  ...
  if (userId || status) {
    ...
    if (status) {
      query += userId ? " AND status=?" : " status=?";
      params.push(status);
    }
  }
...
router.get("/stats/:userId", async (req, res) => {
  const [total] = await pool.query(
    'SELECT COUNT(*) as total, SUM(DATEDIFF(endDate, startDate) + 1) as daysTaken FROM time_off_requests WHERE userId=? AND status="APPROVED"',
    [req.params.userId]
  );
